...
ILI9225::ILI9225(pin rst, pin rs, pin cs, pin sdi, pin clk, pin led) {
	_rst  = rst;
	_rs   = rs;
	_cs   = cs;
	_sdi  = sdi;
...
ILI9225::ILI9225(spi1* _SPI,pin rst, pin rs, pin cs, pin led) {
	SPI=_SPI;
	_rst  = rst;
	_rs   = rs;
	_cs   = cs;
...
void ILI9225::begin(unsigned long _fcpu) {
...
void ILI9225::sSPI(int c)
{
		 for(int bit = 0x8000; bit; bit >>= 1) {
      if(c & bit)	_sdi.high();
	     else 	_sdi.low();
	    _clk.high();
     _clk.low();
...
void ILI9225::_updateCurY(int factor)
{
	...
	curX=(factor>0)?curXMin:curXMax-cfont.width;
		if(curY+cfont.height+lineGap>curYMax)curY=curYMin;
		else if (curY-cfont.height-lineGap<curYMin)curY=curYMax;
}
	...
	for (i = 0; i <= charWidth; i++) {  // each font "column" (+1 blank column for spacing)
		...
		for (j = 0; j < cfont.nbrows; j++) 	{  // each column byte
			...
			for (char k = 0; k < 8; k++) {
				...
				if (charData&(1<<k)) drawPixel(x + i, y + (j * 8) + k, color);
